...
export function MetricCard({ metric, className = "" }: { metric: EnhancedMetric; className?: string }) {
  const Icon = metric.icon;

  return (
    <article className={`${metric.mobile ? "sm:hidden" : ""} ${className} relative min-w-0 overflow-hidden rounded-lg border border-primary/18 bg-card/85 p-5 shadow-sm`}>
      <div className="flex min-w-0 items-start justify-between gap-4">
        ...
        <span className="hidden size-8 shrink-0 place-items-center rounded-full border border-primary/15 bg-background/70 text-primary sm:grid">↗</span>
      </div>
...
function MetricVisual({ type }: { type?: EnhancedMetric["visual"] }) {
  if (type === "bars") {
    return (
      <div className="hidden h-10 shrink-0 items-end gap-1.5 sm:flex" aria-hidden>
  ...
  return (
    <svg viewBox="0 0 92 38" className="hidden h-10 w-24 shrink-0 sm:block" aria-hidden>
